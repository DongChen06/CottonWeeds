...
class ImageSimilarity():
    ...
    def iteration(self, save_header, thresh=0.845, title1=None, title2=None):
        ...
        fields1 = self.load_data_csv(os.path.join(self._tmp_dir, '_' + self._title[0] + '_fields.csv'), delimiter='\t',
                                     include_header=False)
        fields2 = self.load_data_csv(os.path.join(self._tmp_dir, '_' + self._title[1] + '_fields.csv'), delimiter='\t',
                                     include_header=False)
        ...
        start = datetime.datetime.now()
        # feature1.resize((1, np.product(feature1.shape)))
        # feature2.resize((1, np.product(feature2.shape)))
